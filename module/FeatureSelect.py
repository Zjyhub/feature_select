...
class FeatureSelect:
    ...
    def compare(self):
        ...
        for i in range(self.M):
            ...
            print(f"BPSO 本次最优解: \t{bpso_gbest[-1]}, 适应度函数值: {bpso_best_fitness[-1]:.6f}")
            print(f"BPSO_Obl 本次最优解: \t{bpso_obl_gbest[-1]}, 适应度函数值: {bpso_obl_best_fitness[-1]:.6f}")
            print(f"DE 本次最优解: \t\t{de_gbest[-1]}, 适应度函数值: {de_best_fitness[-1]:.6f}")
            print(f"DE_JADE 本次最优解: \t{de_jade_gbest[-1]}, 适应度函数值: {de_jade.global_best_fitness:.6f}")
        
        print(f"\n运行{self.M}次特征选择的结果:")
        print(f"BPSO 平均适应度函数值:\t{np.mean(bpso_best_fitness):.6f} , 最优适应度函数值:{np.min(bpso_best_fitness):.6f}, 最优解:{bpso_gbest[np.argmin(bpso_best_fitness)]}")
        print(f"BPSO_Obl 平均适应度函数值:\t{np.mean(bpso_obl_best_fitness):.6f} , 最优适应度函数值:{np.min(bpso_obl_best_fitness):.6f}, 最优解:{bpso_obl_gbest[np.argmin(bpso_obl_best_fitness)]}")
        print(f"DE 平均适应度函数值:\t{np.mean(de_best_fitness):.6f} , 最优适应度函数值:{np.min(de_best_fitness):.6f}, 最优解:{de_gbest[np.argmin(de_best_fitness)]}")
        print(f"DE_JADE 平均适应度函数值:\t{np.mean(de_jade_best_fitness):.6f} , 最优适应度函数值:{np.min(de_jade_best_fitness):.6f}, 最优解:{de_jade_gbest[np.argmin(de_jade_best_fitness)]}")
