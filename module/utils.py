...
def fitness(
    ...
    for train_index, test_index in kf.split(X):
        ...
        err_rates[i] = 1 - accuracy_score(y_test_kf, y_pred)  # 计算错误率
        i += 1

    dimension = x.shape[0]  # 特征数量
...
# 保存结果到文件
def save_result(
    algorithm_name, accuracy_mean, best_solution, best_accuracy, run_times, Dataset
):
    ...
    with open(
        f"./output/{algorithm_name}/result/{algorithm_name}_{Dataset}.txt", "a"
    ) as f:
        date = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        f.write(
            f"[{date}] {algorithm_name} run {run_times} times , mean accuracy: {accuracy_mean*100:.2f}%, best solution: {best_solution}, best accuracy: {best_accuracy:.2f}%, number of selected features: {best_solution.sum()}/{best_solution.size}\n"
        )
...
def cal_accuracy(train_X, y, x, knn=global_params["knn"], k=5):
    ...
    for train_index, test_index in kf.split(train_X):
        ...
        knn.fit(X_train_kf, y_train_kf)  # 训练
        y_pred = knn.predict(X_test_kf)  # 预测
        accuracy_list[i] = accuracy_score(y_test_kf, y_pred)
        i += 1
