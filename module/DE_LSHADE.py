...
class DE_LSHADE:
    def __init__(self,X,y,iterations=100,init_size=20,min_size=10,alpha=0.99, beta=0.01,u_F=0.5,u_CR=0.5,p=0.1,H=5,r_arc=2.0,mcr_terminal=0.6,max_FES=1000):
        """
        初始化DE算法对象
    ...
    def reduce_population(self):
        ...
        if num_to_remove <= -1:
            ...
            self.pbest_fitness = np.delete(self.pbest_fitness, sorted_index[num_to_remove:])
            self.size += num_to_remove
            len_A = int(self.size*self.r_arc)
            if len(self.A) > len_A:
                self.A = np.delete(self.A,np.random.randint(0,len(self.A),len(self.A)-len_A),axis=0)
    ...
    def init_solution(self):
        ...
        for i in range(self.size):
            ...
            f_new = fitness(self.alpha,self.beta,self.dimension,self.X,self.y,self.P[i],self.knn)
    ...
    def update(self):
        for t in range(self.iterations):
            ...
            for i in range(self.size):
                ...
                # 初始化缩放因子F和交叉概率CR
                self.F=cauchy.rvs(loc=self.M_F[r_i],scale=0.1,size=1)[0] # 从柯西分布中生成F
                ...
                f_x = fitness(self.alpha,self.beta,self.dimension,self.X,self.y,self.P[i],self.knn)
                if f_u < f_x:
